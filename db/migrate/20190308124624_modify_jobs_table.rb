class ModifyJobsTable < ActiveRecord::Migration[5.1]
  ...
  # Add Client Section
  def up
    add_column :jobs, :job_number, :string, unique: true, null: false
    remove_column :jobs, :targeted_amount
    remove_column :jobs, :target_date
    add_column :jobs, :order_number, :string, null: false
    add_column :jobs, :client_section, :string, null: false
  end
